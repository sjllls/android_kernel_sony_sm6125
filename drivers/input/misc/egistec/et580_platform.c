...
#define SDM855_EGIS 1 /* 1:enable ; 0:disable */
...
static int egis_probe(struct platform_device *pdev)
{
	...
	if (!np) {
		...
		if (rc < 0) {
			pr_err("[egis] %s failed to get egistec,gpio_irq",
				__func__);
#if SDM855_EGIS
			egis->irqPin = 118;
			pr_info("[egis] %s set irqPin to %d",
			...
			pr_info("[egis] %s got egistec,gpio_irq %d",
				__func__, rc);
			egis->irqPin = rc;
		}
	}
...
#else
	wake_lock_init(&et580_wake_lock, WAKE_LOCK_SUSPEND, "et580_wake_lock");
#endif
